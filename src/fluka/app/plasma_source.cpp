...
plasma_source::PlasmaSource plasma;
...
void setup_(const double &ionDensityOrigin,
  ...
  // do the appropariate setting
  plasma.setIonDensityOrigin(ionDensityOrigin);
  plasma.setIonTemperatureOrigin(ionTempOrigin);
  plasma.setIonTemperaturePeaking(ionTempPeak);
  plasma.setMajorRadius(majorRadius);
  plasma.setMinorRadius(minorRadius);
  plasma.setElongation(elongation);
  plasma.setTriangularity(triangularity);
  plasma.setShafranovShift(shafranov);
  plasma.setMinToroidalAngle(0.0);
  plasma.setMaxToroidalAngle(360.0);
  plasma.setPlasmaID(0);
  plasma.setReactionType(0);
  plasma.setNumberOfBins(100);
  plasma.setup_plasma_source();
  return; 
}
...
void sample_(double *randoms,
  ...
  std::array<double,8> random_arr;
  std::copy(randoms,randoms+8,random_arr.begin());
  
  plasma.sample(random_arr,x,y,z,
		u,v,w,e);
  x *= 100.; // m -> cm;
  y *= 100.; // m -> cm;
  z *= 100.; // m -> cm;
  e *= 1e-3; // Mev->Gev for fluka
  return;
}
...
PlasmaSource::PlasmaSource(const double ion_density_ped, const double ion_density_sep,
  ...
  ionTemperaturePedestal = ion_temp_ped; // keV
  ionTemperatureSeparatrix = ion_temp_sep; // keV
  ionTemperatureOrigin = ion_temp_origin; // keV
...
void PlasmaSource::setup_plasma_source()
{
  ...
  for (int i = 0 ; i < numberOfBins ; i++) {
    ...
    src_strength.push_back(std::pow(ion_d,2)*xs);
...
void PlasmaSource::sample_energy(const int bin_number, double random_number1, double random_number2,
  ...
  // note ion_kd is in units of sqrt(MeV)
  if ( reactionType == DT ) {
    energy_neutron = (5.59/2.35)*(ion_kt[bin_number])*sample1*sample2;
    energy_neutron += 14.08;
  }
  if ( reactionType == DD ) {
    energy_neutron = (2.655873181/2.35)*(ion_kt[bin_number])*sample1*sample2;
    energy_neutron += 2.54;   
  }
...
/* 
 * sample the dd cross section 
*/
double PlasmaSource::dd_xs(double ion_temp) {
  return sample_nrl_formulary(dd_plasma.xs,ion_temp);
}
